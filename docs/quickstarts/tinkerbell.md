# Lokomotive Tinkerbell quickstart guide
...
* [Step 2: Download Flatcar Container Linux image](#step-2-download-flatcar-container-linux-image)
* [Step 3: Create a cluster configuration](#step-3-create-a-cluster-configuration)
* [Step 4: Deploy the cluster](#step-4-deploy-the-cluster)
...
Before we deploy the cluster, we need to download Flatcar Container Linux image, which will be used as a
base OS image for Tinkerbell provisioner server.
...
For each cluster node, Tinkerbell workflow should be created, which installs OS on node disk. You can see created
...
You can check state of each individual workflow using the following sample commands:
```sh
...
If everything went well, you should see `ACTION_IN_PROGRESS` state on `reboot` action as in the example above. This is expected, as this action
reboots the node, to it has no time to send information about successful execution back to Tinkerbell server.
...
If you see error similar to this, it means that `flatcar_image_path` parameter is not set correctly.
Please make sure that it's value points to the valid Flatcar image.
...
Libvirt is not able to use some path as storage pools. If error similar to this occurs, try
changing the path in `pool_path` attribute to some other value or make sure that UID and GID
mentioned in the error message has permissions to write to configured path.
...
This error is caused by libvirt Terraform provider not being able to
